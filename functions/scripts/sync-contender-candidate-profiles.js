...
import { syncContenderCandidateProfilesFromX } from "../lib/x-sync.js";

const handles = process.argv.slice(2);
const result = await syncContenderCandidateProfilesFromX(getDb(), { handles });

console.log(`Snapshot: ${result.snapshotDate}`);
console.log(`Profile-synced contender candidates: ${result.syncedHandles.join(", ")}`);
